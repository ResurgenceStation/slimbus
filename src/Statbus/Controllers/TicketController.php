...
class TicketController extends Controller {
  ...
  public function getActiveTickets(){
    $tickets = $this->DB->run("
      ...
      FROM ss13ticket t
      JOIN ss13admin AS r ON r.ckey = t.recipient
      JOIN ss13admin AS s ON s.ckey = t.sender
  ...
  public function getSingleTicket(int $round, int $ticket){
    $tickets = $this->DB->run("
      SELECT
        ...
        s.rank as s_rank
      FROM tbl_ticket t
      JOIN tbl_admin AS r ON r.ckey = t.recipient
      JOIN tbl_admin AS s ON s.ckey = t.sender
      WHERE t.round_id = ?
      AND t.ticket = ? 
